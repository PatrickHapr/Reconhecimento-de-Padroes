...
def processar_multiescala(gray, scales=[1.0, 0.8, 1.2]):
    """
    Processa a imagem em múltiplas escalas para detectar feijões de tamanhos diferentes
    """
    detections = []
...
def filtrar_deteccoes_sobrepostas(detections, threshold=0.3):
    ...
    while len(indices) > 0:
        ...
        for i, idx in enumerate(indices[1:]):
            ...
            box_area = (other_box[2] - other_box[0]) * (other_box[3] - other_box[1])
...
def classificar_feijoes(img_original, gray, detections_filtradas):
    """
    Classifica os feijões como bons ou ruins com base em cor e textura
    """
    good_beans = 0
    bad_beans = 0
    ...
    for det in detections_filtradas:
        ...
        x, y, w, h = cv2.boundingRect(contour)
        cv2.putText(img_resultado, label, (x, y - 5),
...
cv2.imwrite("feijoes_classificados_multiescala.jpg", resultado)
